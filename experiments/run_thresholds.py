...
def test_metric_thresholds(X_train, X_test, y_train, y_test, base_params, output_file, pos_weight=1.0):
    """
    ...
    Args:
        ...
        pos_weight: Weight for positive classifier (default: 1.0)
    """
    print("\n" + "="*80)
    ...
    print(f"Fitting classifier with best parameters (pos_weight={pos_weight}) to extract explanations...")
    classifier = LazyFCA(
        pos_params=LazyFCA.Params(
    ...
    # Map metric names to DataFrame column names
    metric_name_mapping = {
    ...
    for metric in METRICS:
        ...
        # Create 10 threshold values for positive and negative
        positive_thresholds = numpy.linspace(positive_scores.min(), positive_scores.max(), 10)
        negative_thresholds = numpy.linspace(negative_scores.min(), negative_scores.max(), 10)
        ...
        threshold_combinations = list(itertools.product(positive_thresholds, negative_thresholds))
        print(f"  Testing {len(threshold_combinations)} threshold combinations")
        
        for pos_threshold, neg_threshold in tqdm.tqdm(threshold_combinations, desc=f"  {metric}"):
            try:
                ...
                print(f"\n  Error with thresholds (pos={pos_threshold:.4f}, neg={neg_threshold:.4f}): {e}")
                continue
    ...
    if all_results:
        ...
        print(f"\n{'='*80}")
        print(f"Saved {len(all_results)} total results across {len(df_all['metric_name'].unique())} metrics to {output_file}")
        print(f"{'='*80}")
    else:
        print(f"\nNo results to save")
...
Examples:
  ...
  python run_thresholds.py data.csv --pos_supporters_covered 5 --neg_supporters_covered 10 --pos_supporter_opposer_ratio 0.36 --neg_supporter_opposer_ratio 4.0 --pos_weight 1.0
        """
    )
    ...
    parser.add_argument('--pos_weight', type=float, default=None,
                        help='Positive weight parameter (default: 1.0 if not provided)')
    ...
    # Use provided pos_weight or default to 1.0
    pos_weight = args.pos_weight if args.pos_weight is not None else 1.0
    test_metric_thresholds(X_train, X_test, y_train, y_test, best_params, metrics_output_file, pos_weight=pos_weight)
